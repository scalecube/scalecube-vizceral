...
import RSocketTransport from './RSocketTransport';
...
class TrafficFlow extends React.Component {
  ...
  schedule () {
    setTimeout(this.beginSampleData, 1000);
  }

  beginSampleData () {
    self.traffic = { nodes: [], connections: [] };
    const transport = new RSocketTransport('ws://localhost:9090/');
    transport.connect().subscribe({
      onComplete: (socket) => {
              // socket provides the rsocket interactions fire/forget, request/response,
              // request/stream, etc as well as methods to close the socket.
        console.log('Connected!');
        transport.next(socket, JSON.stringify({ metadata: { q: '/io.scalecube.vizceral.region/listen' } }));
      },
      onError: error => console.error(error)
    });

    transport.listen().subscribe((next) => {
      ...
      /*
    request.get('https://raw.githubusercontent.com/ronenhamias/api-sandbox-data/master/data.json?' + Date.now())
      .set('Accept', 'application/json')
      .end((err, res) => {
        if (res && res.status === 200) {
          self.traffic.clientUpdateTime = Date.now();
          self.updateData(JSON.parse(res.text));
          self.handlePopState();
          self.render();
          self.schedule();
        }
